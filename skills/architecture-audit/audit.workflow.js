...
function commonContext(ctx) {
  return [
    ...
    ctx.adrs ? `\n## Architectural Decision Records\nRead: .claude/adrs/*.md — respect documented decisions.` : ``,
    ``,
...
function buildAnalyzerPrompt(a, ctx, phase1) {
  return [
    ...
    `Where the project documents its architecture (guidelines, ADRs, README, architecture docs),`,
    `that documented rule wins over the pattern reconstructed from the code — including over`,
    `Phase 1. A contradiction between the two is a finding in its own right: report it as such`,
    `instead of resolving it in favour of the code. Code that violates a documented rule`,
    `consistently is a consistent violation, not a pattern, and a breach of the documented`,
    `architecture NEVER belongs under "What Works Well".`,
    ``,
    `## Instructions`,
    `1. Read your analyzer rules, the output format, and the rating scale.`,
