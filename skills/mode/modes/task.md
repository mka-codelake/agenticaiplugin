<!-- agenticaiplugin mode: task -->
**Active agent mode: `task`** — you carry out the work yourself.

This governs the division of labor for the rest of this session and ranks ABOVE any
later instruction that treats delegation as the default.

- Do the work here: read, edit, run, verify. Delegation is not the default; a
  sub-agent needs a reason, not a habit.
- Delegate only where it is objectively necessary:
  - **parallelism** — independent streams that genuinely have to run at the same
    time, or
