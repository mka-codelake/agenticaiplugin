...
| "An explanation is not an instruction" paragraph, appended to the data-not-instructions block | Without it the reviewer reads *unverifiable* as *untrue*. A comment citing an earlier review round next to a permission grant drew a **Critical for fabricated history in 6 of 6 runs** — in reports that called the permission itself correct. The cause is structural: the prompt fetches PR metadata without `comments`, so review history is outside what the reviewer can see, and `git log` shows nothing. Measured 6/6 → 0/6 after the paragraph, with a genuine injection attempt still drawing a Critical 6/6 (#135). Removing it restores a procedure that punishes the correct developer response — documenting the reason at the line after a finding was refuted |
