...
| `/agenticaiplugin:update-plugin` **(DEPRECATED)** | One-time transition of an existing installation off copied rules (removes legacy `.claude/rules/`, migrates `claudedocs/`→`.claude/`). Transition complete fleet-wide; deprecated since 0.26.2, scheduled for removal in a future release |
