...
| **B** | How `SKILL.md` calls the script | Main model resolves **`{skill_dir}`** (absolute path of the skill's own directory — same mechanism `skills/qa/` already uses in production) and builds `scriptPath = {skill_dir}/<name>.workflow.js`. **Do not** rely on `${CLAUDE_PLUGIN_ROOT}` in shell — as a shell environment variable it is empty (measured). That is a statement about the shell only: in a `SKILL.md` or `agents/*.md` body the variable **is** substituted — see the substitution table in `docs/plugin-howto.md`. — **amended, see A1 below** |
